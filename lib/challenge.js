...
function prompt7() {
  ...
  for(i = 0; i <= 100; i++){
    if(i % 5 == 0 && i > 1){
    console.log(`I found a ${i}! High Five Baby!`);
    }
  }
}
...
function prompt8() {
  ...
  // YOUR CODE HERE
  if(someNumber < 30){
    console.log("That's a small number you got there...");
  }else if(someNumber < 60 && someNumber > 30){
    console.log("The number is average");
  }else{
    console.log("This is really big!");
  }
...
function prompt9() {
  ...
  for(let i = 0; i < starWars.length; i++){
    console.log(`Number ${i + 1}: ${starWars[i]}`);
  }
}
 ...
 */
  ...
  // if(nums.length % 2 === 0){ //array is even, average of middle 2 numbers
  //   //find middle
  //   let medianEven = Math.floor(nums.length/2)
  //   //find median 
  //   median = (nums)
  ...
  //   let medianOdd = math.floor((nums.length / 2)); //Gives you the index
  //   medianOdd = nums[medianOdd];
...
function prompt11() {
    ...
    let median; // Declare the median variable
  // YOUR CODE HERE
  if(nums.length % 2 === 0) {//if it's even
    median = (nums[nums.length / 2 - 1] + (nums[nums.length / 2]) / 2) ;

  } else { //it's odd 9 items / 2 would be 4.5, we would round down to 4
    median = nums[Math.floor(nums.length/2)];

  }
  console.log(median); //Used chat GPT for assistence, need some help understanding what this is doing exactly outside of the nums.length % 2 === 0
}

prompt11();
// COMMENT IN THE BELOW LINE OF CODE TO TEST YOUR OUTPUT IN THE BROWSER!

// CHATGPT explanation of why math.floor() is used
...
// In this example, the nums array has a length of 5, which is odd. So, in the else block, Math.floor(nums.length / 2) calculates 2.5, which is rounded down to 2. This gives us the index of the middle element in the array, which is nums[2], or 3. Since this is an odd-length array, 3 is the median, and it is logged to the console.
...
// In this example, the if block would be executed because nums.length % 2 would equal 0. In the if block, median would be calculated as the average of the two middle elements, which are 3 and 4. The median would be (3 + 4) / 2 = 3.5, and it would be logged to the console.
...
function prompt13() {
  ...
  for(i=0;i<nestedArrays.length; i++) {
    for(j=0; j<nestedArrays[i].length; i++){
