...
function prompt2() {
  for(i = 1; i < 100; i++){
    if(i % 2 === 0){
      ...
      console.log("I'm sorry that is an odd number");
    }
  }
}
...
//first print each index name-- as it counts up the array, it should tell us what that is.
function prompt9() {
  //lets do a for loop
// for(i = 0; i < 6; i++){
...
//   i++
// }
//lets now print their name with the index
// let i = 0
// while(i < 6){
...
//I need to sort the numbers-- lets say there are odd numbers, then I need to figure out the middle number post sort-- and present that, if they are even i need to add the two middle numbers divide them and present that.
//using multiple sources here-- geeksforgeeks, w3resources and fellow classmates to figure out sorting and capturing the median -- ive given up on this one.
  function prompt11(){
    // let median = Math.floor(newMid)
    // let newMid = nums[(nums.length)/2] + nums[((nums.length)/2)+1]
    // let median = nums;
    // median = median.sort()
    // console.log(median);
    // var length = median.length;
    // if(length % 2 == 1) {
    //   console.log(median[(length / 2) - .5])
    //   return median[(length / 2) - .5]
    // }else{
    //   console.log(median[length / 2] + median[(length / 2) - 1] / 2);
    //   return(median[length / 2] + median[(length / 2) - 1]) / 2;
    // }
  }
  // return nums.length % 2 !== 0 new[med] : (new[med - 1] + new[med])/2;
...
function prompt13(arrOfArrays) {
  for( i = 0; i < arrOfArrays.length; i++){
    for(j = 0; j < arrOfArrays[i].length; j++) {
